...
bot.onText(/\/start/, async (msg) => {
    ...
    if (msg.chat.type === 'group' || msg.chat.type === 'supergroup') {
        ...
        await bot.sendMessage(chatId, 'Welcome to the Automated Trading Bot! Choose your mode:', opts);
    }
});
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    if (mode === 'signal') {
        ...
        if (groups.length === 0) {
            await bot.sendMessage(getUserId(), 'No groups found. Please add the bot to a group first.');
            return;
        }
...
async function sendHomeMessage(chatId) {
    ...
    await bot.sendMessage(chatId, 'Redirecting to home. Please choose your mode:', opts);
}
